...
class W2v():
    ...
    def train(self, corpus_path, size = 100, min_count = 1, window = 5, iter = 20, out_path = './word2vec.model'):
        ...
        if self.__model is None:
            self.__model = word2vec.Word2Vec(sentences, size = size, min_count = min_count, window = window, iter = iter)
        else:
            self.__model.build___vocab(sentences,update=True)
            self.__model.train(sentences, total_examples = self.__model.corpus_count, epochs = self.__model.iter)
        self.__vocab_index = self.__model.wv.index2word
